...
void static HoneyMiner(CWallet *pwallet)
{
    LogPrintf("HoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("honey-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running HoneyMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HoneyMiner terminated\n");
        throw;
    }
}

void GenerateHoneys(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&HoneyMiner, pwallet));
}
#endif

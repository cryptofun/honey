...
{
    CHoneyAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            scriptPubKey.SetDestination(CHoneyAddress(rcp.address.toStdString()).Get());
...
{
    QString strAddress = QString::fromStdString(CHoneyAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for (const COutput& out : vCoins)
    {
        ...
        mapCoins[CHoneyAddress(address).ToString().c_str()].push_back(out);
    }
}

...
{
    QString ret = QString("honey:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HONEY unit input in lnReqAmount, we generate the URI with HONEY as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(HoneyUnits::format(HoneyUnits::HONEY, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

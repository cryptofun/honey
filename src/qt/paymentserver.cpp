...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(HONEY_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(HONEY_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(HONEY_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click honey: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start honey: click-to-pay handler");
    else
...
{
    // clicking on honey: URLs creates FileOpen events on the Mac:
